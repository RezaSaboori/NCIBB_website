...
// Import shaders
import ambientVert from '../shaders/ambient-particle.vert?raw';
import ambientFrag from '../shaders/ambient-particle.frag?raw';
...
export function createAmbientParticleRenderer(gl, config, headerSize, headerCenter) {
  ...
  async function init() {
    ...
    program = createProgramFromSources(gl, ambientVert, ambientFrag);
    ...
    uniforms = {
      ...
      uAmbientOpacity: gl.getUniformLocation(program, 'u_ambientOpacity'),
  ...
  function update(time) {
    updateCounter++;
    // Throttle updates to every 2 frames to reduce CPU load
  ...
  function render(modelMatrix, viewMatrix, projectionMatrix, pointSize, mousePos, opacity = 1.0) {
    if (!program) return;
    ...
    if (type1Particles && configRef.ambientType1Enabled) {
      ...
      gl.uniform1f(uniforms.uAmbientOpacity, opacity);
