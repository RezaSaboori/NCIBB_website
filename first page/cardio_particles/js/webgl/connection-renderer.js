...
export function createConnectionRenderer(gl, config, particles, particleCount, center, maxSize) {
    ...
    async function init() {
        ...
        program = await createProgram(
            gl,
            `../shaders/heart-connection.vert${cacheBuster}`,
            `../shaders/heart-connection.frag${cacheBuster}`
        );
     ...
     * Render connections
     */
    function render(modelMatrix, viewMatrix, projectionMatrix, time) {
        ...
        gl.uniform1f(uniforms.uConnectionOpacity, config.connectionOpacity);
