...
void main() {
    ...
    // ========================================
    // DEPTH OF FIELD CALCULATION - DISABLED FOR HEART
    // ========================================
    if (false && u_dofEnabled) { // THIS BLOCK IS NOW DISABLED
        // Calculate Circle of Confusion (CoC) using thin lens equation
        float depthDiff = abs(depth - u_dofFocalDistance);
        
        // Simplified CoC calculation for real-time rendering
        float cocScale = (u_dofFocalLength / 50.0) / (u_dofAperture * 0.2);
        v_coc = (depthDiff / (u_dofFocalDistance + 0.1)) / cocScale;
        
        // Calculate blur amount based on near/far fields
        float nearBlur = 0.0;
        float farBlur = 0.0;
        ...
        // FORCE MAXIMUM BLUR FOR TESTING (remove this after debugging)
        // v_blurAmount = 5.0;
        
        // Calculate depth-based effects factor
        float normalizedDepth = clamp((depth - u_dofFocalDistance) / 5.0, 0.0, 1.0);
        v_depthFactor = normalizedDepth;
        
        // Increase particle size for out-of-focus particles (bokeh effect)
        float bokehSizeMultiplier = 1.0 + v_blurAmount * 2.0 * u_dofBokehScale;
        particleScale *= bokehSizeMultiplier;
    } else {
        v_coc = 0.0;
        v_blurAmount = 0.0;
        v_depthFactor = 0.0;
    ...
    // Point size based on depth, particle size, mouse interaction, and bokeh
