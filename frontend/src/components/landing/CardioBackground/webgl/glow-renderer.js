...
// Import shaders
import glowVert from '../shaders/background-glow.vert?raw';
import glowFrag from '../shaders/background-glow.frag?raw';
...
export function createGlowRenderer(gl, config) {
    ...
    async function init() {
        ...
        program = createProgramFromSources(gl, glowVert, glowFrag);
     ...
     */
    function render(modelMatrix, viewMatrix, projectionMatrix, heartCenter, heartSize, opacityMultiplier = 1.0) {
        ...
        // Set uniforms - apply opacity multiplier to intensity for fade effect
        gl.uniform2f(uniforms.uHeartScreenPos, screenX, screenY);
        gl.uniform1f(uniforms.uIntensity, config.glowIntensity * opacityMultiplier);
        ...
        // Set up blending for additive glow (Pre-multiplied Alpha)
        gl.enable(gl.BLEND);
