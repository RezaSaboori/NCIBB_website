...
const CardioBackground = forwardRef(({ 
  ...
  backgroundColor = '#070707', // Background color for the canvas rendering
  ...
  onLoadingProgress, // Callback for loading progress (0-100)
  onInitialized, // Callback when WebGL is fully ready
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        if (onLoadingProgress) onLoadingProgress(10); // Start loading
        ...
        if (!particleDataRef.current) {
          ...
          const meshUrl = `${assetBase}landing/webgl_asset/mesh_data.json`;
          
          if (onLoadingProgress) onLoadingProgress(20);
          const response = await fetch(meshUrl);
          if (!response.ok) throw new Error(`Failed to load mesh data: ${response.status}`);
          const manifest = await response.json();
          if (onLoadingProgress) onLoadingProgress(40);
        ...
        if (particleDataRef.current && particleDataRef.current.PARTICLE_COUNT === PARTICLE_COUNT) {
          ...
          if (onLoadingProgress) onLoadingProgress(60);
        ...
        if (onLoadingProgress) onLoadingProgress(80);
        ...
        console.error("CardioBackground initialization error:", error);
        setIsSupported(false);
        if (onError) onError(error.message || "Failed to initialize 3D background");
      } finally {
        isInitializingRef.current = false;
      }
    };
